...
def run_base_experiment(config: dict, run_id: int, algo_config: dict, data_bundle: dict):
    """
    ...
    try:
        ...
        # Simple progress bar for the inner loop
        pbar = tqdm(range(num_timesteps), desc=f"Algo: {algo_name}, Run: {run_id+1}", leave=False)
        for t in pbar:
            row = data.iloc[t]
        ...
        print(f"\nERROR in experiment for {algo_name}, run {run_id+1}:\n{traceback.format_exc()}")
        return None
...
def main(config_path: str):
    ...
    if 'algorithms' not in config:
        raise ValueError("This script requires the config to have an 'algorithms' list.")
    ...
    # --- Stage 2: Sequential Experiment Execution ---
    num_runs = config['num_runs']
    algorithms_to_run = config['algorithms']
    total_tasks = num_runs * len(algorithms_to_run)
    print(f"\n--- STAGE 2: Sequential Experiment Execution ---")
    print(f"Total tasks to run: {total_tasks} ({num_runs} runs x {len(algorithms_to_run)} algorithms)")

    overall_pbar = tqdm(total=total_tasks, desc="Overall Progress")
    
    for run_id in range(num_runs):
        for algo_config in algorithms_to_run:
            run_base_experiment(config, run_id, algo_config, data_bundle)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run base bandit algorithm comparison experiments sequentially.")
    parser.add_argument(
    ...
    try:
        ...
        print(f"\n--- ERROR: Experiment for {args.config} failed: ---")
        traceback.print_exc()
